...
export class KeyController {
  ...
  keyHandler(e) {
    ...
    const scenes = {
      ...
      draft: this.main.draftScene,
    };

    const allScenes = Object.values(scenes);
    const anySceneOpen = (except = null) => allScenes.some((s) => s.isOpen && s !== except);

    const gameLocked = () =>
      this.main.defeatScene.isOpen ||
      this.main.newGameScene.isOpen ||
      this.main.tutorialScene.isOpen ||
      this.main.draftScene.isOpen ||
      this.main.finalScene.isOpen;

    const gameplayLocked = () => gameLocked() || this.main.game.deployingUnit;
    ...
    switch (e.key) {
      case 'Escape': {
        ...
        if (this.main.pokemonScene.nameChange.isOpen)
          return this.main.pokemonScene.nameChange.close();
        ...
        if (this.main.profileScene.deleteRecord.isOpen)
          return this.main.profileScene.deleteRecord.close();
      ...
      case langKeys.leftArrow:
        if (this.main.pokemonScene.isOpen && !this.main.pokemonScene.nameChange.isOpen)
          this.main.pokemonScene.changePokemon(-1);
        break;
