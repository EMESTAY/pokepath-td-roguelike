...
export class Enemy extends Sprite {
  constructor(x, y, enemy, waypoints, main, ctx) {
    ...
    if (this.main.area.inChallenge[4]) {
      ...
      this.hpMax = Math.floor(this.hpMax * 1.5);
      this.armor = Math.floor(this.armor * 1.5);
      this.armorMax = Math.floor(this.armorMax * 1.5);
    }
  ...
  draw() {
    ...
    if (
      this.main.displayHealth == 2 ||
      (this.main.displayHealth == 1 && this.main.area.waveNumber == 100)
    ) {
      this.ctx.save();
  ...
  getDamaged(
    ...
    if (pokemon?.item?.id == 'stickyBarb' && this.statusEffects.length > 0) {
      this.statusEffects.forEach((effect) => {
        if (effect.type == 'stun' || effect.type == 'slow') amount = Math.ceil(amount * 1.15);
      });
    }
    ...
    if (pokemon?.item?.id == 'badgeOfHonor') {
      amount += Math.floor(amount * (this.main.player.stars / 30 / 100));
    }

    if (amount >= 9999 && source == 'physical') this.main.player.unlockAchievement(18);
    if (amount > this.main.player.stats.highestHit && source == 'physical')
      this.main.player.stats.highestHit = amount;
    ...
    if (
      ...
      ability?.id === 'stanceChange' ||
      pokemon?.item?.id == 'shieldBreakerBullet'
    )
      armorDamage *= 2;

    if (pokemon?.item?.id == 'nanabBerry') armorDamage *= 1.5;

    if (
      (ability?.id === 'heal' && pokemon?.item?.id != 'lifeOrb' && Math.random() < 0.025) ||
      (pokemon?.item?.id == 'shellBell' &&
        pokemon.damageDealt > 50000 &&
        !this.main.area.shellBellWaveUsed) ||
      (pokemon?.item?.id == 'clefairyDoll' &&
        pokemon.damageDealt > 25000 &&
        !this.main.area.clefairyDollUsed)
    ) {
      ...
      if (pokemon?.item?.id == 'clefairyDoll' && pokemon.damageDealt > 25000)
        this.main.area.clefairyDollUsed = true;
      if (pokemon?.item?.id == 'shellBell' && pokemon.damageDealt > 50000)
    ...
    if (ability?.id === 'greed') {
      ...
      if (this.main.player.achievementProgress.stolenGold >= 100000)
    ...
    if (this.hp <= 0 && !this.dying) {
      ...
      if (this.enemy.id == 'delibird') {
        ...
        if (this.main.player.achievementProgress.delibirdCount >= 225)
      ...
      if (this.main.player.stats.defeatedSpecies.size === 100)
    ...
    if (
      (ability?.id === 'curse' ||
        ability?.id === 'willOWisp' ||
        ability?.id === 'curseDoubleShot') &&
      this.cursed &&
      cursedDamage > 0 &&
      !alreadyCursed.has(this)
    ) {
      ...
      this.main.area.enemies.forEach((e) => {
        ...
        if (ability?.id === 'willOWisp' && e.canBurn && e.cursed) {
          if (pokemon?.item?.id == 'falmeOrb')
            this.applyStatusEffect({ type: 'burn', damagePercent: 0.00625, duration: 10 }, pokemon);
          else
    ...
    this.statusEffects.forEach((effect) => {
      ...
      if (effect.type === 'burn') {
        while (effect.timer >= interval && effect.duration > 0) {
          this.getDamaged(
            Math.ceil(this.hpMax * effect.damagePercent),
            'burn',
            null,
            false,
            new Set(),
