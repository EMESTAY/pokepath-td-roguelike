...
export class MapScene extends GameScene {
  ...
  render() {
    ...
    this.routeContainer = new Element(this.container, {
      className: 'maps-scene-route-container',
    }).element;
    this.routes = [];

    for (let i = 0; i < 9; i++) {
      this.routes[i] = new Element(this.routeContainer, {
        className: 'maps-scene-route',
        image: routeData[i].background,
      }).element;

      this.routes[i].name = new Element(this.routes[i], {
        className: 'maps-scene-route-name',
        text: routeData[i].name[this.main.lang].toUpperCase(),
      }).element;
      this.routes[i].recordContainer = new Element(this.routes[i], {
        className: 'maps-scene-route-record-container',
      }).element;
      this.routes[i].record = new Element(this.routes[i], {
        className: 'maps-scene-route-record',
      }).element;
      this.routes[i].requires = new Element(this.routes[i], {
        className: 'maps-scene-route-requires',
      }).element;
      this.routes[i].addEventListener('click', () => {
        this.changeMap(i);
      });
      this.routes[i].addEventListener('mouseenter', () => {
        playSound('hover2', 'ui');
      });
    }
  ...
  displayBaseRoutes() {
    for (let i = 0; i < 9; i++) {
      ...
      this.routes[i].record.innerHTML =
        `<span class="msrre">⭐</span>${Math.min(100, this.main.player.records[i])}`;
    }
  }
