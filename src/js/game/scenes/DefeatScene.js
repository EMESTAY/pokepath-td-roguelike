...
export class DefeatScene extends GameScene {
  ...
  update() {
    ...
    this.title.innerHTML = text.defeat.title[this.main.lang].toUpperCase();
    this.prompt.innerHTML = `${text.defeat.prompt[
      this.main.lang
    ...
    //(this.main.player.stars > 400) ? this.banette.style.display = 'block' : this.banette.style.display = 'none';
  }

  open(challengeLost = false) {
    super.open();
    this.update();
    this.main.game.stop();
    ...
    if (this.main.area.goldWave > this.main.player.stats.maxGoldPerWave[0]) {
      ...
      this.main.player.stats.maxGoldPerWave[1] = this.main.area.getRouteTag(
    ...
    if (goldPerSecond > this.main.player.stats.maxGoldPerTime[0]) {
      ...
      this.main.player.stats.maxGoldPerTime[1] = this.main.area.getRouteTag(
        this.main.area.routeNumber,
        this.main.area.waveNumber
      );
    }
    ...
    if (this.main.autoReset == 1 && !challengeLost) {
      this.restart({
        autoWave: this.main.area.autoWave,
        speedBuff: this.main.game.speedFactor,
      });
    }
    if (this.main.autoReset == 2 && !challengeLost) {
      if (this.main.area.waveNumber > 25)
        this.retry({
          autoWave: this.main.area.autoWave,
          speedBuff: this.main.game.speedFactor,
  ...
  // 	if (data.usedBanette) location.reload();
  // 	else {
  // 		super.close();
