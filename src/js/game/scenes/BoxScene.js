...
export class BoxScene extends GameScene {
  ...
  render() {
    ...
    this.unitSelectedName = new Element(this.container, {
      className: 'box-scene-unit-selected-name',
    }).element;
    ...
    this.favoriteButton = new Element(this.container, {
      className: 'box-scene-favorite-button',
      text: '⭐',
    }).element;
    ...
    this.unitContainer = new Element(this.container, {
      className: 'box-scene-unit-container',
    }).element;
    this.units = [];

    this.buttonContainer = new Element(this.container, {
      className: 'box-scene-button-container',
    }).element;
    ...
    this.dataUnit.addEventListener('click', () => {
      this.main.pokemonScene.open(this.selected, this.selectedPos, this.pokemon);
    });
    ...
    this.dataUnit.addEventListener('mouseenter', () => {
      playSound('hover3', 'ui');
    });
    this.addUnit.addEventListener('mouseenter', () => {
      playSound('hover3', 'ui');
    });
    this.removeUnit.addEventListener('mouseenter', () => {
      playSound('hover3', 'ui');
    });
    this.removeAll.addEventListener('mouseenter', () => {
      playSound('hover3', 'ui');
    });

    for (let i = 0; i < 90; i++) {
      this.units[i] = new Element(this.unitContainer, { className: 'box-scene-unit' }).element;
      this.units[i].text = new Element(this.units[i], {
        className: 'box-scene-unit-text stroke',
      }).element;
      this.units[i].fav = new Element(this.units[i], { className: 'box-scene-unit-fav' }).element;
      this.units[i].shiny = new Element(this.units[i], {
        className: 'box-scene-unit-shiny',
      }).element;
        ...
        if (this.selected.inGroup) {
          ...
          if (this.selected.isDeployed) {
            ...
            const index = this.main.area.towers.findIndex(
              (tower) => tower.pokemon == this.selected
            );
      ...
      this.units[i].addEventListener('mouseenter', () => {
        playSound('hover1', 'ui');
      });
    }

    this.sortContainer = new Element(this.container, {
      className: 'box-scene-sort-container',
    }).element;

    this.sortArrowLeft = new Element(this.sortContainer, {
      className: 'box-scene-sort-arrow',
      text: '<',
    }).element;
    this.sortValue = new Element(this.sortContainer, { className: 'box-scene-sort-value' }).element;
    this.sortArrowRight = new Element(this.sortContainer, {
      className: 'box-scene-sort-arrow',
      text: '>',
    }).element;
    this.sortArrowLeft.addEventListener('click', () => {
      this.changesort(-1);
    });
    this.sortArrowRight.addEventListener('click', () => {
      this.changesort(1);
    });
    this.sortArrowLeft.addEventListener('mouseenter', () => {
      playSound('hover1', 'ui');
    });
    this.sortArrowRight.addEventListener('mouseenter', () => {
      playSound('hover1', 'ui');
    });

    this.tabContainer = new Element(this.container, {
      className: 'box-scene-tab-container',
    }).element;
    this.tabs = [];
    ...
    for (let i = 0; i < TAB_CONTENT.length; i++) {
      ...
      this.tabs[i].addEventListener('mouseenter', () => {
        playSound('hover1', 'ui');
      });
    }
  ...
  searchByName() {
    ...
    this.searchPokemon = this.pokemon.filter(
      (poke) =>
  ...
  displayUnits() {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      if (poke) {
        ...
        if (this.sorted <= 1 || this.sorted == 10)
          this.units[i].text.innerHTML = poke.name[this.main.lang];
        else if (this.sorted == 2) {
          if (this.main.area.inChallenge[0]) this.units[i].text.innerHTML = `Lv 50`;
          else this.units[i].text.innerHTML = `Lv ${poke.lvl}`;
        } else if (this.sorted == 3)
          this.units[i].text.innerHTML =
            `<span style="line-height: 7px;">${abilityData[poke.ability.id].name[this.main.lang]}</span>`;
        else if (this.sorted > 3 && this.sorted < 7) {
          poke.tiles.forEach((tile) => {
            if (tile == 2)
              this.units[i].text.innerHTML +=
                `<span style="color: #409552; font-size: 7px; letter-spacing: 2px;">G</span>`;
            if (tile == 3)
              this.units[i].text.innerHTML +=
                `<span style="color: #5ea1d9; font-size: 7px; letter-spacing: 2px;">W</span>`;
            if (tile == 4)
              this.units[i].text.innerHTML +=
                `<span style="color: #b89184; font-size: 7px; letter-spacing: 2px;">M</span>`;
          });
        }
        ...
        if (this.tabSelected != 0) {
          ...
          switch (this.tabSelected) {
            ...
            case 3:
              ...
              break;
            case 4:
              if (![58, 59, 63, 64, 65, 66].includes(poke.id))
                unit.style.filter = 'brightness(0.3)';
              break;
          }
        }
      } else {
    ...
    this.pokemon.forEach((pokemon, i) => {
      ...
      switch (tab) {
        ...
        case 3:
          ...
          break;
        case 4:
          if (![58, 59, 63, 64, 65, 66].includes(pokemon.id))
            this.units[i].style.filter = 'brightness(0.3)';
          break;
      }
    });
  }
  ...
  displayPokemon() {
    this.unitSelectedName.innerText =
      this.selected.alias != undefined
        ? this.selected.alias.toUpperCase()
        : this.selected.name[this.main.lang].toUpperCase();
    this.unitSelectedName.innerText += this.main.area.inChallenge[0]
      ? ` [50]`
      : ` [${this.selected.lvl}]`;
  ...
  open() {
    if (this.main.area.inChallenge[1]) return;
