...
export class ProfileScene extends GameScene {
  ...
  render() {
    ...
    this.playerContainer = new Element(this.container, {
      className: "profile-player-container",
    }).element;

    this.portrait = new Element(this.playerContainer, {
      className: "profile-scene-portrait",
    }).element;

    this.avatarPrev = new Element(this.playerContainer, {
      className: "profile-scene-avatar-arrow-prev",
      text: "<",
    }).element;
    this.avatarNext = new Element(this.playerContainer, {
      className: "profile-scene-avatar-arrow-next",
      text: ">",
    }).element;
    ...
    this.achievementsContainer = new Element(this.container, {
      className: "profile-scene-achievements-container",
    }).element;
    this.achievement = [];

    for (let i = 0; i < 32; i++) {
      this.achievement[i] = new Element(this.achievementsContainer, {
        className: "profile-scene-achievement",
      }).element;
    }

    this.statsContainer = new Element(this.container, {
      className: "profile-stats-container",
    }).element;
    this.stats = [];

    for (let i = 0; i < 21; i++) {
      this.stats[i] = new Element(this.statsContainer, {
        className: "profile-stat",
      }).element;
      this.stats[i].label = new Element(this.stats[i], {
        className: "profile-stat-label",
      }).element;
      this.stats[i].value = new Element(this.stats[i], {
        className: "profile-stat-value",
      }).element;
    }

    this.stats[19].value.addEventListener("click", () => {
      this.deleteRecord.open(19);
    });
    this.stats[20].value.addEventListener("click", () => {
      this.deleteRecord.open(20);
    });
  ...
  update() {
    ...
    this.achievement.forEach((achievement, i) => {
      this.achievement[
        i
      ].style.backgroundImage = `url("${achievementData[i].image}")`;
    ...
    for (let i = 0; i < 21; i++) {
      this.stats[i].label.innerText =
        text.profile.stats[i][this.main.lang].toUpperCase();
    }

    this.stats[0].value.innerText = this.main.utility.minutsToTime(
      this.main.player.stats.timePlayed
    );
    this.stats[1].value.innerText = `${Math.min(
      900,
      this.main.player.stars
    )}/900`;
    this.stats[2].value.innerText = `${this.main.player.stats.pokemonOwned}/90`;
    this.stats[3].value.innerText = `${this.main.player.shinyAmount}/90`;
    this.stats[4].value.innerText = `${this.main.player.stats.highestPokemonLevel}/100`;
    this.stats[5].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.totalPokemonLevel,
      this.main.lang
    )}/${this.main.utility.numberDot(9000, this.main.lang)}`;
    this.stats[6].value.innerText = `$${this.main.utility.numberDot(
      this.main.player.stats.totalGold,
      this.main.lang
    )}`;
    this.stats[7].value.innerText = `${this.main.player.itemAmount}/63`;
    this.stats[8].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.wavesCompleted,
      this.main.lang
    )}`;
    this.stats[9].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.highestHit,
      this.main.lang
    )}`;
    this.stats[10].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.defeatedEnemies,
      this.main.lang
    )}`;
    this.stats[11].value.innerText = `${this.main.player.stats.defeatedSpecies.size}/124`;
    this.stats[12].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.appliedStuns,
      this.main.lang
    )}`;
    this.stats[13].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.appliedSlows,
      this.main.lang
    )}`;
    this.stats[14].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.appliedBurns,
      this.main.lang
    )}`;
    this.stats[15].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.appliedPoisons,
      this.main.lang
    )}`;
    this.stats[16].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.appliedCurses,
      this.main.lang
    )}`;
    this.stats[17].value.innerText = `${this.main.utility.numberDot(
      this.main.player.stats.resets,
      this.main.lang
    )}`;
    this.stats[18].value.innerText = `$${this.main.utility.numberDot(
      this.main.player.achievementProgress.stolenGold,
      this.main.lang
    )}`;
    this.stats[19].value.innerText =
      this.main.player.stats.maxGoldPerWave[1] == null
        ? `$0`
        : `(${
            this.main.player.stats.maxGoldPerWave[1]
          }) $${this.main.utility.numberDot(
            this.main.player.stats.maxGoldPerWave[0],
            this.main.lang
          )}`;
    this.stats[20].value.innerText =
      this.main.player.stats.maxGoldPerTime[1] == null
        ? `$0/s`
        : `(${
            this.main.player.stats.maxGoldPerTime[1]
          }) $${this.main.utility.numberDot(
            this.main.player.stats.maxGoldPerTime[0],
            this.main.lang
          )}/s`;
  }
...
export class DeleteRecord extends GameScene {
  ...
  update() {
    this.title.innerHTML =
      text.profile.stats[this.stat][this.main.lang].toUpperCase();
