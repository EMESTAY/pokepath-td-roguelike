...
export class PokemonScene extends GameScene {
  ...
  setBlocked(flag) {
    ...
    if (this.container) {
      ...
      this.container.style.filter = this.isBlocked
        ? 'grayscale(60%) brightness(0.95)'
        : 'revert-layer';
      this.container
        .querySelectorAll('button, a, [tabindex], input, [role="button"]')
        .forEach((el) => {
          if (this.isBlocked) {
  ...
  render() {
    ...
    this.name.addEventListener('click', () => {
      this.nameChange.open(this.pokemon);
    });

    this.arrowPrev = new Element(this.container, {
      className: 'pokemon-scene-arrow-prev',
      text: '<',
    }).element;
    this.arrowNext = new Element(this.container, {
      className: 'pokemon-scene-arrow-next',
      text: '>',
    }).element;
    ...
    this.arrowPrev.addEventListener('mouseenter', () => {
      playSound('hover1', 'ui');
    });
    this.arrowNext.addEventListener('mouseenter', () => {
      playSound('hover1', 'ui');
    });

    this.dataContainer = new Element(this.container, {
      className: 'pokemon-scene-data-container',
    }).element;
    this.data = [];

    DATA.forEach((data) => {
      this.data[data] = new Element(this.dataContainer, {
        className: 'pokemon-scene-data',
      }).element;
      this.data[data].label = new Element(this.data[data], {
        className: 'pokemon-scene-data-label',
        text: text.pokemon[data][this.main.lang],
      }).element;
      if (data != 'attackType')
        this.data[data].value = new Element(this.data[data], {
          className: 'pokemon-scene-data-value',
        }).element;
      else {
        this.data[data].left = new Element(this.data[data], {
          className: 'pokemon-scene-data-arrow-at',
          text: '<',
        }).element;
        this.data[data].value = new Element(this.data[data], {
          className: 'pokemon-scene-data-value-at',
        }).element;
        this.data[data].right = new Element(this.data[data], {
          className: 'pokemon-scene-data-arrow-at',
          text: '>',
        }).element;

        this.data[data].left.addEventListener('click', () => {
          this.changeAttackType(-1);
        });
        this.data[data].right.addEventListener('click', () => {
          this.changeAttackType(1);
        });
        this.data[data].left.addEventListener('mouseenter', () => {
          playSound('hover1', 'ui');
        });
        this.data[data].right.addEventListener('mouseenter', () => {
          playSound('hover1', 'ui');
        });
      }
    });

    this.abilityContainer = new Element(this.container, {
      className: 'pokemon-scene-ability-container',
    }).element;
    this.abilityName = new Element(this.abilityContainer, {
      className: 'pokemon-scene-ability-name',
    }).element;
    this.abilityDescription = new Element(this.abilityContainer, {
      className: 'pokemon-scene-ability-description',
    }).element;

    this.itemContainer = new Element(this.container, {
      className: 'pokemon-scene-item-container',
    }).element;
    this.itemName = new Element(this.itemContainer, {
      className: 'pokemon-scene-item-name',
    }).element;
    this.itemDescription = new Element(this.itemContainer, {
      className: 'pokemon-scene-item-description',
    }).element;
    this.itemIcon = new Element(this.itemContainer, {
      className: 'pokemon-scene-item-icon',
    }).element;
    this.itemIcon.addEventListener('click', () => {
      if (!this.isBlocked) this.itemWindow.open(this.pokemon);
    });

    this.levelUpContainer = new Element(this.container, {
      className: 'pokemon-scene-level-up-container',
    }).element;
    this.levelUp = new Element(this.levelUpContainer, {
      className: 'pokemon-scene-level-up',
    }).element;
    ...
    this.levelUpFive = new Element(this.levelUpContainer, {
      className: 'pokemon-scene-level-up',
    }).element;
    ...
    this.levelUpTen = new Element(this.levelUpContainer, {
      className: 'pokemon-scene-level-up',
    }).element;
    ...
    this.levelUp.addEventListener('mouseleave', () => {
      this.update();
    });
    this.levelUpFive.addEventListener('mouseleave', () => {
      this.update();
    });
    this.levelUpTen.addEventListener('mouseleave', () => {
      this.update();
    });

    this.buttonDNA = new Element(this.container, { className: 'pokemon-scene-button-dna' }).element;
    this.buttonDNA.addEventListener('mouseenter', () => {
      playSound('hover2', 'ui');
    });
    ...
    this.buttonChangeForm = new Element(this.container, {
      className: 'pokemon-scene-button-dna',
    }).element;
    this.buttonChangeForm.addEventListener('mouseenter', () => {
      playSound('hover2', 'ui');
    });
    ...
    this.evolutionSprite = new Element(this.container, {
      className: 'pokemon-scene-evolution-sprite',
    }).element;
    this.evolutionLevel = new Element(this.evolutionSprite, {
      className: 'pokemon-scene-evolution-level',
    }).element;
  ...
  update() {
    ...
    DATA.forEach((data) => {
      this.data[data].label.innerText = text.pokemon[data][this.main.lang];
    });
    ...
    if (!this.main.area.inChallenge[0])
      this.name.innerHTML =
        this.pokemon.alias != undefined
          ? `${this.pokemon.alias.toUpperCase()} [${this.pokemon.lvl}]`
          : `${this.pokemon.name[this.main.lang].toUpperCase()} [${this.pokemon.lvl}]`;
    else
      this.name.innerHTML =
        this.pokemon.alias != undefined
          ? `${this.pokemon.alias.toUpperCase()} [50]`
          : `${this.pokemon.name[this.main.lang].toUpperCase()} [50]`;
    ...
    this.data['rangeType'].value.innerHTML =
      `${text.pokemon[this.pokemon.rangeType][this.main.lang]}`;
    this.data['attackType'].value.innerHTML =
      `${TARGET_MODES_TRADUCTIONS[this.pokemon.targetMode][this.main.lang]}`;
    ...
    if (this.pokemon.id == 64 || this.pokemon?.adn?.id == 64) {
      ...
      this.pokemon.id == 2 ||
      this.pokemon?.adn?.id == 2 ||
      this.pokemon.id == 45 ||
      this.pokemon?.adn?.id == 45 ||
      this.pokemon.id == 72 ||
      this.pokemon?.adn?.id == 72
    ) {
    ...
    if (this.pokemon.tiles.length == 4)
      this.data['terrain'].value.innerHTML = TERRAINS[5][this.main.lang];
    else {
    ...
    if (this.pokemon?.item?.id == 'silphScope' && this.pokemon.id != 53) {
      this.pokemon.changeTargetMode(TARGET_MODES[20]);
      this.data['attackType'].value.innerHTML =
        `${TARGET_MODES_TRADUCTIONS[this.pokemon.targetMode][this.main.lang]}`;
    } else if (
      this.pokemon?.item?.id != 'silphScope' &&
      this.pokemon.targetMode == 'invisible' &&
    ...
    if (this.pokemon.specie.evolution != undefined && this.pokemon.id != 70) {
      ...
      if (this.pokemon.id != 76)
        ...
        if (this.main.utility.isBetweenHours(8, 18))
          this.evolutionSprite.style.backgroundImage = `url("${pokemonData['lycanrocDay'].sprite.base}")`;
        else
          this.evolutionSprite.style.backgroundImage = `url("${pokemonData['lycanrocNight'].sprite.base}")`;
      }
    ...
    if (this.pokemon.id == 76 && this.pokemon.lvl == 100) {
      ...
      if (this.pokemon.ability.id == 'toughClawsDay')
        this.buttonChangeForm.style.backgroundImage = `url("${pokemonData['lycanrocNight'].sprite.base}")`;
      else
        this.buttonChangeForm.style.backgroundImage = `url("${pokemonData['lycanrocDay'].sprite.base}")`;
      if (this.pokemon.isDeployed) {
  ...
  changeAttackType(dir) {
    ...
    let indexMax =
      this.pokemon.ability.id == 'frisk' || this.pokemon?.item?.id == 'silphScope' ? 20 : 19;
    let indexMin = 0;
  ...
  open(pokemon, pos, boxArray = [], isBlocked = false) {
    super.open();
    this.background.style.backgroundColor = this.main.boxScene.isOpen
      ? 'rgba(0, 0, 0, 0)'
      : 'rgba(0, 0, 0, 0.6)';
  ...
  showLevelUpEffect(levels = 1) {
    const specie =
      this.pokemon.specie.evolution != undefined &&
      this.pokemon.lvl >= this.pokemon.specie.evolution.level - levels
        ? pokemonData[this.pokemon.specie.evolution.pokemon]
        : this.pokemon.specie;

    const newPower = Math.floor(
      specie.power.base + specie.power.scale * (this.pokemon.lvl + levels)
    );
    const newSpeed = Math.floor(
      specie.speed.base + specie.speed.scale * (this.pokemon.lvl + levels)
    );
    const newCritical = specie.critical.base + specie.critical.scale * (this.pokemon.lvl + levels);
    const newRange = Math.floor(
      specie.range.base + specie.range.scale * (this.pokemon.lvl + levels)
    );

    const powerDiff = newPower - this.pokemon.power;
    const speedDiff = Math.abs(
      (newSpeed / 1000).toFixed(2) - (this.pokemon.speed / 1000).toFixed(2)
    ).toFixed(2);
    ...
    if (powerDiff > 0) {
      this.data['power'].value.innerHTML =
        `${this.pokemon.power} <span style="color:var(--green)">(+${powerDiff})</span>`;
    }
    if (speedDiff > 0) {
      this.data['speed'].value.innerHTML =
        `${(this.pokemon.speed / 1000).toFixed(2)}s <span style="color:var(--green)">(-${speedDiff}s)</span>`;
    }
    if (criticalDiff > 0) {
      this.data['critical'].value.innerHTML =
        `${this.pokemon.critical.toFixed(1)}% <span style="color:var(--green)">(+${criticalDiff}%)</span>`;
    }
    if (rangeDiff > 0) {
      this.data['range'].value.innerHTML =
        `${this.pokemon.range} <span style="color:var(--green)">(+${rangeDiff})</span>`;
    }
  }
  ...
  updateStatsChanges() {
    ...
    if (this.pokemon.isDeployed) {
      ...
      if (
        (tower.tile &&
          (tower.tile.land === 2 ||
            (tower.tile.land == 1 && tower.pokemon?.item?.id == 'fertiliser')) &&
          tower.pokemon.ability.id === 'toughClawsNight') ||
        (tower.tile &&
          tower.tile.land === 4 &&
          (tower.pokemon.ability.id === 'toughClawsDay' ||
            tower.pokemon.ability.id === 'toughClaws'))
      )
    ...
    switch (this.pokemon?.item?.id) {
      ...
      case 'choiceScarf':
        if (
          this.pokemon.ability.id === 'quadraShot' ||
          this.pokemon.ability.id === 'quadraShotSand'
        )
          mulSpeed *= 1 - 0.6;
    ...
    if (this.pokemon.id == 66) {
      ...
      const rate = this.pokemon?.item?.id === 'rockyHelmet' ? 0.1 : 0.05;
    ...
    if (this.pokemon.isDeployed) {
      ...
      if (tower) {
        ...
        if (
          [3, 4, 5].includes(this.main.area.routeNumber) &&
          (towerAbility === 'doubleShotSand' || towerAbility === 'quadraShotSand')
        ) {
          mulRange *= 2;
        }
...
class ItemWindow {
  ...
  render() {
    ...
    for (let i = 0; i < 63; i++) {
      ...
      this.slot[i].equiped = new Element(this.slot[i], {
        className: 'item-scene-slot-equiped stroke',
        text: 'E',
      }).element;
    }

    this.removeItem = new Element(this.container, {
      className: 'item-scene-slot item-scene-slot-x',
      text: 'X',
    }).element;
  ...
  update() {
    ...
    this.main.player.items.forEach((item, i) => {
      ...
      else if (item.equipedBy != undefined)
