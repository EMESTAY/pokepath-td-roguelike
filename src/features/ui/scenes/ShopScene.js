...
export class ShopScene extends GameScene {
  ...
  render() {
    ...
    this.itemContainer = new Element(this.container, {
      className: 'shop-scene-item-container',
    }).element;

    this.egg = new Element(this.itemContainer, {
      className: 'shop-scene-item shop-scene-egg',
    }).element;
    ...
    this.egg.addEventListener('mouseenter', () => {
      playSound('hover3', 'ui');
    });
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.itemSlot[i].purchaseButton = new Element(this.itemSlot[i], {
        className: 'shop-scene-purchase',
      }).element;
      this.itemSlot[i].addEventListener('mouseenter', () => {
        playSound('hover3', 'ui');
      });
      this.itemSlot[i].addEventListener('click', () => {
        this.main.shop.buyItem(i);
      });
    }
  ...
  updateEgg() {
    ...
    if (
      this.main.player.gold >= Math.min(50000, this.main.shop.eggPrice) &&
      this.main.shop.eggList.length > 0
    ) {
  ...
  updateItems() {
    this.itemSlot.forEach((item, i) => {
      ...
      if (this.main.shop.itemStock[i] != null) {
        ...
        if (this.main.shop.itemStock[i].price > 99999999)
          item.purchaseButton.style.fontSize = '6.5px';
        else if (this.main.shop.itemStock[i].price > 999999)
          item.purchaseButton.style.fontSize = '8px';
