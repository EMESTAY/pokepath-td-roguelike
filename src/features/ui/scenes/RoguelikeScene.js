...
import { itemData } from '../../../data/static/itemData.js';
...
const rewardStrategies = {
  ...
  candy: (scene, reward) => {
    for (let i = 0; i < 3; i++) {
      const member =
        scene.main.team.pokemon[Math.floor(Math.random() * scene.main.team.pokemon.length)];
...
export class RoguelikeScene extends GameScene {
  ...
  finishSelection() {
    this.close();
    if (this.main.area && typeof this.main.area.onWaveEndComplete === 'function') {
      // NOTE: Call the NEW completion handler to avoid recursive loops
      this.main.area.onWaveEndComplete();
    } else {
      console.error('onWaveEndComplete not found in Area');
    }
  }
  ...
  open() {
    ...
    this.rerollCount = 0;
    if (this.rerollBtn) this.rerollBtn.innerText = `REROLL (-${this.rerollCost}g)`;
  ...
  generatePokemonReward() {
    ...
    const existing = this.main.team.pokemon.find((p) => p.specie.id === spec.id);
    if (existing) {
    ...
    if (spec.ability && spec.ability.name && spec.ability.description) {
      ...
      abilityDesc = spec.ability.description[this.main.lang] || spec.ability.description[0];
    }
