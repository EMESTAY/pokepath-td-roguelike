...
export class ChallengeScene extends GameScene {
  ...
  render() {
    ...
    this.routeName = new Element(this.container, {
      className: 'challenge-scene-route-name',
    }).element;

    this.challengeRow = [];
    this.challengeListContainer = new Element(this.container, {
      className: 'challenge-scene-list-container',
    }).element;

    CHALLENGES_LIST.forEach((label, i) => {
      this.challengeRow[i] = new Element(this.challengeListContainer, {
        className: 'challenge-scene-list-row',
      }).element;
      this.challengeRow[i].label = new Element(this.challengeRow[i], {
        className: 'challenge-scene-list-row-label',
      }).element;
      this.challengeRow[i].result = new Element(this.challengeRow[i], {
        className: 'challenge-scene-list-row-result',
      }).element;
    ...
    this.challengeInfoContainer = new Element(this.container, {
      className: 'challenge-scene-info-container',
    }).element;
    this.challengeInfoText = new Element(this.challengeInfoContainer, {
      className: 'challenge-scene-info-text',
    }).element;

    this.challengeStartButton = new Element(this.container, {
      className: 'challenge-scene-start-button',
    }).element;
    this.challengeStartButton.addEventListener('click', () => {
      this.startChallenge();
    });

    this.challengeWarning = new Element(this.container, {
      className: 'challenge-scene-warning',
    }).element;
  ...
  update() {
    ...
    this.challenges.forEach((chall, i) => {
      this.challengeRow[i].label.innerHTML =
        text.challenge[CHALLENGES_LIST[i]].title[this.main.lang].toUpperCase();
      this.challengeRow[i].result.innerHTML = this.challenges[i]
    ...
    this.challengeInfoText.innerHTML =
      text.challenge[CHALLENGES_LIST[this.challengeSelected]].description[
        this.main.lang
      ].toUpperCase();
    ...
    if (this.main.player.stars < 900 || this.main.box.pokemon.length < 30) {
      ...
      if (this.main.area.inChallenge) {
        this.challengeStartButton.innerHTML =
          text.challenge.surrender[this.main.lang].toUpperCase();
      }
    }
  }
