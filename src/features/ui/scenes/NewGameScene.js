...
export class NewGameScene extends GameScene {
  ...
  update() {
    ...
    if (this.gameMode === 0) {
      this.modeRow.value.innerText = text.ui.classic[this.main.lang].toUpperCase();
      this.starterContainer.style.display = 'flex';
      this.starterName.style.display = 'block';
      this.starterDescription.innerText =
        STARTER[this.starterSelected].ability.description[this.main.lang].toUpperCase();
      this.selectStarter(this.starterSelected);
    } else {
      this.modeRow.value.innerText = text.ui.draft[this.main.lang].toUpperCase();
      this.starterContainer.style.display = 'none';
      this.starterName.style.display = 'none';
      this.starterDescription.innerText = text.ui.draftDesc[this.main.lang].toUpperCase();
  ...
  selectStarter(pos) {
    ...
    STARTER.forEach((starter, i) => {
      ...
      if (pos == i)
        this.starters[i].style.filter = `brightness(1) drop-shadow(0 0 4px ${starter.color})`;
    });
  }
